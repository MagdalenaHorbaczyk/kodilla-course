...
public class Statistics implements BookStatistics {
    @Override
    public int averagePublicationYear(Map<BookSignature, BookLB> books) {
        if (books.size() == 0) return 0;
        int sum = 0;
        for (Map.Entry<BookSignature, BookLB> entry : books.entrySet()) {
    ...
    @Override
    public int medianPublicationYear(Map<BookSignature, BookLB> books) {
        if (books.size() == 0) return 0;
